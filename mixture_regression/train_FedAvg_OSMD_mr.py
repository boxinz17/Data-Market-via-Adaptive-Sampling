...
learning_rate = 1e-3  # parameter of OSMD
...
w_est, n_access, util_list, est_err_list, time_used = train_FedAvg_OSMD_mr(providers_train_list, val_data, w_true, step_size_global, batch_size, n_commun, learning_rate, alpha)

print("Time used: {}(s)".format(time_used))

# store the result
with open('result/OSMD/w_est_rdseed=' + str(rd_seed_array[i_rand]) + '.pickle', 'wb') as f:
    pickle.dump(w_est, f)

with open('result/OSMD/n_access_rdseed=' + str(rd_seed_array[i_rand]) + '.pickle', 'wb') as f:
    pickle.dump(n_access, f)

with open('result/OSMD/util_list_rdseed=' + str(rd_seed_array[i_rand]) + '.pickle', 'wb') as f:
    pickle.dump(util_list, f)

with open('result/OSMD/est_err_list_rdseed=' + str(rd_seed_array[i_rand]) + '.pickle', 'wb') as f:
    pickle.dump(est_err_list, f)

with open('result/OSMD/time_used_rdseed=' + str(rd_seed_array[i_rand]) + '.pickle', 'wb') as f:
